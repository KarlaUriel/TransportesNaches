...
#agregarGasto, #btnVolver,#menuButton, #sidebarMenuButton, .fuel-level-btn,\n\
 .btnAgregarDestino, .btnEliminarDestino, #registrarOtroViaje, #gestionAdmin, #gestionSubmenu, #btnGestion)').forEach(element => {
...
//    }
    ...
    const isNewTrip = urlParams.get('newTrip') === 'true';

    if (isNewTrip) {
        // Forzar formulario de inicio y limpiar idNota
    ...
    try {
        ...
        if (viajePendienteSelect) {
            ...
            pendingTrips.forEach(trip => {
                ...
                option.textContent = `Viaje a ${trip.destino} (Cliente: ${trip.cliente}), iniciada por: ${trip.operador}`;
        ...
        if (notasIncompletas.length > 0) {
            ...
            const destinosPendientes = pendingTrips.map(trip => `${trip.destino} (Cliente: ${trip.cliente}) Iniciada por: ${trip.operador}`).join(', ');
            Swal.fire({
...
async function iniciarViaje(event) {
    ...
    const nombreOperador = localStorage.getItem("nombreCompleto").trim();
    const now = new Date();
...
async function finalizarViaje(event) {
    ...
    try {
        ...
        // Actualizar pendingTrips: eliminar el viaje finalizado
        let pendingTrips = JSON.parse(localStorage.getItem('pendingTrips') || '[]');
        pendingTrips = pendingTrips.filter(trip => trip.idNota !== idNota);
        localStorage.setItem('pendingTrips', JSON.stringify(pendingTrips));

        // Restaurar pendingTrips desde originalPendingTrips
        const originalPendingTrips = localStorage.getItem('originalPendingTrips');
        if (originalPendingTrips) {
            localStorage.setItem('pendingTrips', originalPendingTrips);
            console.log('pendingTrips restaurado con:', originalPendingTrips);
            localStorage.removeItem('originalPendingTrips');
        }

        // Restaurar nombreCompleto desde originalNombreCompleto
        const originalNombreCompleto = localStorage.getItem('originalNombreCompleto');
        if (originalNombreCompleto) {
            localStorage.setItem('nombreCompleto', originalNombreCompleto);
            localStorage.removeItem('originalNombreCompleto');
            const nombreOperadorInput = document.getElementById('nombreOperador');
            if (nombreOperadorInput) {
                nombreOperadorInput.value = originalNombreCompleto;
                console.log('nombreCompleto restaurado a:', originalNombreCompleto);
            }
        }
        const originalidUsuario = localStorage.getItem('originalIdUsuario');
        if (originalidUsuario) {
            localStorage.setItem('idUsuario', originalidUsuario);
            localStorage.removeItem('originalIdUsuario');
        }
    ...
    if (hayDatosNoGuardados()) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                // Restaurar pendingTrips y nombreCompleto
                const originalPendingTrips = localStorage.getItem('originalPendingTrips');
                if (originalPendingTrips) {
                    localStorage.setItem('pendingTrips', originalPendingTrips);
                    localStorage.removeItem('originalPendingTrips');
