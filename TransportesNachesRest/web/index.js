...
async function cargarLogin(){
   let url="http://localhost:8080/TransportesNaches/modules/Login/view_Login.html";
   let resp = await fetch(url);
   let contenido = await resp.text();
   document.getElementById('contenedor_login').innerHTML = contenido;
   cm = await import("http://localhost:8080/TransportesNaches/modules/Login/controller_Login.js");
   cm.inicializar();
}