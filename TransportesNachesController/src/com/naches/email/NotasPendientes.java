...
public class NotasPendientes {
    ...
    public List<NotaGasto> getPendingNotes() throws Exception {
        ...
        String query = "SELECT ng.*, cn.*"
                + " FROM notaGasto ng"
                + " INNER JOIN contabilidadN cn ON ng.idNota = cn.idNota"
                + " INNER JOIN cliente c ON ng.idCliente = c.idCliente"
                + " WHERE cn.estadoFact = 'Pendiente'"
                + " AND ng.fechaLlegada >= '2025-06-01'"
                + " AND c.factura = 1;";

        ConexionMySQL conexion = new ConexionMySQL();
        try (Connection conn = conexion.open(); Statement stmt = conn.createStatement(); ResultSet rs = stmt.executeQuery(query)) {
            while (rs.next()) {
